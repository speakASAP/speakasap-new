...
**Status:** Refactoring complete; verification (Step 7) done  
...
- [x] **7.1** Smoke test: open each main URL (/, /german/, /winners, /winners/:id, /about, /profile, /steps/:id) and confirm correct content and no console errors.
- [ ] **7.2** Verify redirect chain: speakasap.com/marathon, speakasap.com/marathon/german/ → marathon.alfares.cz with correct path (optional manual check).
- [x] **7.3** Deploy: build frontend, build Docker image (includes `public/`), deploy to dev/prod per runbook; no nginx changes in prod (config regenerated by deploy script).
- [ ] **7.4** Monitor logs (marathon service, portal shim) for errors; fix critical issues and document known limitations (ongoing).

**Done when:** Production marathon.alfares.cz serves full UI; all critical user flows work; doc updated with any deferred items. **Done 2026-02-22:** Deployed; browser smoke test passed for /, /de/, /winners, /about, /reviews, /profile, /register, /awards, /support, /rules, /faq, /api/v1/health. See MARATHON_VERIFICATION_CHECKLIST.md (Test run table).
...
No implementation; procedure only. Use when rollback is required.
...
- **Verification:** Use **MARATHON_VERIFICATION_CHECKLIST.md** for Step 7 (smoke test URLs, redirect chain, deploy, monitor). After full rollout, complete 7.1, 7.2, 7.4.
