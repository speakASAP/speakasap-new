...
const ROLE = 'internal:payment-service:internal';

function contextFor(headers: Record<string, string>, rolesMeta: { roles: string[] } | undefined) {
  const req = { headers };
  return {
    getHandler: () => ({}),
    getClass: () => ({ name: 'SalaryDisburseController' }),
  ...
  it('accepts Auth RS256 bearer with required role', async () => {
    globalThis.fetch = jest.fn(async () =>
      new Response(
        JSON.stringify({
          valid: true,
          user: { id: 'svc-1', email: 'svc-salary-service--payment-service@internal.alfares.cz', roles: [ROLE] },
        }),
